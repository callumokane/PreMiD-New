...
module.exports = {
    ...
    run: async (client, out) => {
        ...
        let cmd = client.command.find(c => c.config.name == out.d.data.name && c.config.slashCommand), perms = client.elevation(out.d.member.user.id);
        ...
        cmd.run(null, null, null, data, await perms);
    }
}